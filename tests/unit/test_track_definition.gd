...
func test_kitchen_definition_validates() -> void:
	...
	assert_equal(definition.route_points.size(), 21, "Kitchen should use the sectioned countertop raceway route")
	assert_equal(definition.checkpoint_indices, [0, 6, 14, 18], "Kitchen checkpoints should follow the sectioned countertop route")
