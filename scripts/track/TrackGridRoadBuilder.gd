...
static func build_grid_collision_mesh(layout: Dictionary) -> ArrayMesh:
	...
	for key in footprint.keys():
		...
		_append_grid_surface_quad(vertices, indices, origin, grid_basis, cell_size, tile_data, [
			Vector3(x0, 0.0, z0),
			Vector3(x1, 0.0, z0),
			Vector3(x0, 0.0, z1),
			Vector3(x1, 0.0, z1),
...
static func _append_grid_surface_quad(vertices: PackedVector3Array, indices: PackedInt32Array, origin: Vector3, grid_basis: Basis, cell_size: Vector3, tile_data: Dictionary, local_points: Array) -> void:
	...
	for local_value in local_points:
		...
		world.y = _surface_y_for_grid_local_point(tile_data, local, cell_size)
...
static func _boundary_wall_segment(a: Vector3, b: Vector3, outward: Vector3, wall_height: float, wall_thickness: float) -> Dictionary:
	...
	var bottom_y := minf(a.y, b.y) - BOUNDARY_WALL_SKIRT_DEPTH
	var center := a.lerp(b, 0.5)
	center.y = bottom_y + shape_height * 0.5
	var position := center - z_axis * (thickness * 0.25)
	return {
		"position": position,
		"basis": basis,
