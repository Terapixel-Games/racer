...
const HOME_YARD_MODE_DIR := "res://assets/gameplay/tracks/home_yard_v3/modes"
...
func test_kitchen_metadata_matches_server_shape() -> void:
	...
	assert_equal(str(metadata.get("version", "")), HOME_YARD_VERSION, "Metadata should include the shared home-yard package version")
	...
	assert_true((metadata.get("stage_props", []) as Array).size() >= 5, "Metadata should export stage props")
	assert_true((metadata.get("stage_interactions", []) as Array).size() >= 2, "Metadata should export stage interactions")
...
func test_home_yard_metadata_exports_stage_interactions() -> void:
	for track_id in ["kitchen", "attic", "bedroom", "garden", "glam_closet", "outdoor_playground", "playroom", "sandbox"]:
...
func test_track_catalog_uses_cooked_package_metadata() -> void:
	...
	assert_equal(str(package.get("map_id", "")), HOME_YARD_MAP_ID, "Catalog package should point Kitchen at the shared home-yard v3 map")
	...
	assert_equal(str(package.get("metadata_path", "")), "%s/kitchen_track_metadata.json" % HOME_YARD_MODE_DIR, "Catalog should expose exported server metadata")
	assert_equal(str(package.get("version", "")), HOME_YARD_VERSION, "Catalog should expose the package version")
