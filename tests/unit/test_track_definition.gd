...
func test_kitchen_definition_validates() -> void:
	...
	assert_equal(definition.route_points.size(), 10, "Kitchen should use the countertop perimeter route")
	assert_equal(definition.checkpoint_indices, [0, 3, 6, 8], "Kitchen checkpoints should follow the countertop route")
