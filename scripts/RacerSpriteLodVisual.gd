...
func configure(texture: Texture2D, manifest: Dictionary = {}) -> void:
	...
	_sprite.frame = 0
	var sprite_world_size := float(manifest.get("sprite_world_size", 1.75))
	_sprite.pixel_size = sprite_world_size / float(frame_size)
	_sprite.position.y = sprite_world_size * 0.5
