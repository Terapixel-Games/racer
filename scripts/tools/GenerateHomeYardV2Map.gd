...
const MAP_ID := "home_yard_v2"
const MAP_DISPLAY_NAME := "Racer House + Yard"
const VERSION := "home_yard_gambrel_v2_2026_05_12"
const BASE_DIR := "res://assets/gameplay/tracks/home_yard_v2"
const MODE_DIR := "res://assets/gameplay/tracks/home_yard_v2/modes"
const MAP_SCENE_PATH := "res://assets/gameplay/tracks/home_yard_v2/home_yard_v2_map.tscn"
const MAP_DEFINITION_PATH := "res://assets/gameplay/tracks/home_yard_v2/home_yard_v2_track_map.tres"
...
const PLAN_CONTRACT := {
	...
	"selected_alternative": "Dutch Gambrel V2",
	"site_orientation": "front/street south, backyard north",
	"style_contract": "toy-scale Dutch Colonial suburban house with a full-width gambrel roof, legible porch, garage/service edge, patio-to-yard hinge, upper bedroom/glam rooms, and Popper attic route inside the roof volume",
	...
	"roof_contract": "Dutch gambrel roof: lower steep roof planes spring from the upper wall plate, upper shallow planes meet at one ridge, attic route clearance is validated inside the roof section, and no rectangular attic story may be visible above the roof.",
}
...
func _add_attic_interior(root: Node3D, parent: Node3D) -> void:
	parent.set_meta("plan_role", "Popper high-ramp attic route inside measured Dutch gambrel roof volume; exterior gables and roof are owned by Roof")
	...
	_add_box(root, finishes, "AtticRafterLeftA", Vector3(-46, 130, 18), Vector3(3, 4, 118), Color(0.20, 0.14, 0.10), false, 0, Vector3(0, 0, -22))
	_add_box(root, finishes, "AtticRafterRightA", Vector3(166, 130, 18), Vector3(3, 4, 118), Color(0.20, 0.14, 0.10), false, 0, Vector3(0, 0, 22))
	_add_box(root, finishes, "AtticRidgeBeamInterior", Vector3(60, 146, 18), Vector3(190, 4, 5), Color(0.18, 0.12, 0.08), false)
	_add_box(root, finishes, "PopperHighRampLaunchDeck", Vector3(-16, 119, -14), Vector3(46, 6, 26), Color(0.42, 0.22, 0.10), false)
	_add_box(root, finishes, "PopperHighRampLandingDeck", Vector3(132, 123, 48), Vector3(52, 8, 28), Color(0.46, 0.26, 0.12), false)
	_add_box(root, finishes, "PopperBankedCardboardRamp", Vector3(54, 121, 18), Vector3(118, 4, 24), Color(0.72, 0.42, 0.18), false, 22.0)
	_add_box(root, finishes, "PopperRafterGateA", Vector3(-28, 126, 50), Vector3(6, 22, 6), Color(0.16, 0.10, 0.06), false)
	_add_box(root, finishes, "PopperRafterGateB", Vector3(148, 130, -12), Vector3(6, 26, 6), Color(0.16, 0.10, 0.06), false)
...
func _add_exterior_architecture(root: Node3D, parent: Node3D) -> void:
	parent.set_meta("style_contract", "toy-scale Dutch Colonial suburban house: rectangular body, broad gambrel roof, flared eaves, ordered front windows, compact entry porch, practical garage/service side, warm siding, cream trim")
	...
	_add_box(root, parent, "DutchFrontEntrySidingField", Vector3(-48, 57, 112), Vector3(112, 24, 6), siding, false)
	...
	_add_window(root, parent, "GambrelAtticFrontVentWindow", Vector3(60, 126, 133), Vector3(30, 16, 1.0))
	_add_box(root, parent, "FrontGutterRun", Vector3(0, 71, 140), Vector3(452, 3, 4), Color(0.12, 0.12, 0.11), false)
	_add_box(root, parent, "BackGutterRun", Vector3(0, 71, -118), Vector3(452, 3, 4), Color(0.12, 0.12, 0.11), false)
...
func _add_exterior_wall_system(root: Node3D, parent: Node3D) -> void:
	...
	_add_wall_z(root, parent, "ExteriorFrontWallLeft", 106, -214, -70, exterior, true, 0.0, 72.0)
	_add_wall_z(root, parent, "ExteriorFrontWallEntryHeader", 106, -28, 94, exterior, true, 0.0, 72.0)
	_add_wall_z(root, parent, "ExteriorFrontGarageWall", 106, 94, 214, exterior, true, 0.0, 72.0)
	_add_wall_z(root, parent, "ExteriorBackWallWest", -86, -214, -75, exterior, true, 0.0, 72.0)
	_add_wall_z(root, parent, "ExteriorBackPatioHeader", -86, -15, 62, exterior, true, 0.0, 72.0)
	_add_wall_z(root, parent, "ExteriorBackGarageWall", -86, 62, 214, exterior, true, 0.0, 72.0)
	_add_wall_x(root, parent, "ExteriorWestWall", -214, -86, 106, exterior, true, 0.0, 72.0)
	_add_wall_x(root, parent, "ExteriorEastGarageWall", 214, -86, 106, exterior, true, 0.0, 72.0)
	_add_box(root, parent, "MainRoofSoffitClosure", Vector3(0, 72.5, 10), Vector3(432, 4, 202), exterior.darkened(0.10), false)
...
func _add_roof_system(root: Node3D, parent: Node3D) -> void:
	...
	parent.set_meta("massing_contract", "single Dutch Colonial gambrel roof over the primary house body with subordinate garage cross-gable and porch hood; Popper attic is inside the gambrel envelope with no stacked attic box")
	parent.set_meta("roof_contract", {
		"dutch_gambrel": {"span_axis": "x", "eave_y": 72.0, "break_y": 118.0, "ridge_y": 150.0, "break_x": 150.0, "ridge_x": 0.0, "overhang": 14.0, "attic_floor_y": 116.0, "attic_route_max_y": 128.0},
	...
	_add_roof_plane_x(root, parent, "DutchGambrelLowerLeftPlane", -236, -150, -122, 144, 72, 118, roof.darkened(0.02))
	_add_roof_plane_x(root, parent, "DutchGambrelUpperLeftPlane", -150, 0, -122, 144, 118, 150, roof)
	_add_roof_plane_x(root, parent, "DutchGambrelUpperRightPlane", 0, 150, -122, 144, 150, 118, roof)
	_add_roof_plane_x(root, parent, "DutchGambrelLowerRightPlane", 150, 236, -122, 144, 118, 72, roof.darkened(0.02))
	_add_box(root, parent, "DutchGambrelRidgeCap", Vector3(0, 151, 11), Vector3(8, 5, 266), shadow, false)
	_add_box(root, parent, "DutchGambrelLeftBreakCap", Vector3(-150, 119, 11), Vector3(8, 4, 266), shadow.lightened(0.04), false)
	_add_box(root, parent, "DutchGambrelRightBreakCap", Vector3(150, 119, 11), Vector3(8, 4, 266), shadow.lightened(0.04), false)
	_add_gambrel_gable_wall_z(root, parent, "DutchGambrelFrontGableWall", 132, -224, 224, 44, 72, 118, 150, siding.lightened(0.02))
	_add_gambrel_gable_wall_z(root, parent, "DutchGambrelBackGableWall", -110, -224, 224, 44, 72, 118, 150, siding.darkened(0.06))
	...
	_add_box(root, parent, "GambrelFrontEaveFascia", Vector3(0, 73, 144), Vector3(472, 6, 6), shadow, false)
	_add_box(root, parent, "GambrelBackEaveFascia", Vector3(0, 73, -122), Vector3(472, 6, 6), shadow, false)
	_add_box(root, parent, "GambrelWestRakeFascia", Vector3(-236, 86, 11), Vector3(6, 7, 266), shadow, false)
	_add_box(root, parent, "GambrelEastRakeFascia", Vector3(236, 86, 11), Vector3(6, 7, 266), shadow, false)
	_add_box(root, parent, "GambrelSoffitFront", Vector3(0, 70, 140), Vector3(464, 4, 12), shadow.lightened(0.18), false)
	_add_box(root, parent, "GambrelSoffitBack", Vector3(0, 70, -118), Vector3(464, 4, 12), shadow.lightened(0.12), false)
...
func _zone_contract_for_course(course_id: String) -> Dictionary:
	match course_id:
		...
		"attic":
			return {"zone_id": "gambrel_attic_high_ramp_zone", "center": Vector3(60, 116, 18), "size": Vector3(214, 58, 88), "floor_top_y": 116.60, "roof_clearance_max_y": 150.0}
...
func _route_cells_for_course(course_id: String) -> Array[Vector3i]:
	match course_id:
		...
		"glam_closet":
			return _compact_vertical_route()
		"attic":
			return _route_from_points([Vector3i(-5, 0, -2), Vector3i(5, 0, -2), Vector3i(5, 1, 2), Vector3i(-5, 1, 2)])
	return []
...
func _add_gambrel_gable_wall_z(root: Node3D, parent: Node3D, node_name: String, z: float, x0: float, x1: float, base_y: float, eave_y: float, break_y: float, ridge_y: float, color: Color) -> Node3D:
	...
	parent.add_child(holder)
	holder.owner = root
	_add_box(root, holder, "WallBelowGambrelEave", Vector3((x0 + x1) * 0.5, (base_y + eave_y) * 0.5, z), Vector3(absf(x1 - x0), eave_y - base_y, 6), color, false)
	...
	_add_mesh(root, holder, "GambrelGableUpperWall", vertices, PackedInt32Array([0, 1, 2, 0, 2, 4, 2, 3, 4]), color.darkened(0.06))
	_add_box(root, holder, "GambrelLeftRakeTrim", Vector3((x0 + left_break) * 0.5, (eave_y + break_y) * 0.5, z + 0.4), Vector3(5, 4, 7), color.lightened(0.16), false, 0, Vector3(0, 0, -28))
	_add_box(root, holder, "GambrelRightRakeTrim", Vector3((x1 + right_break) * 0.5, (eave_y + break_y) * 0.5, z + 0.4), Vector3(5, 4, 7), color.lightened(0.16), false, 0, Vector3(0, 0, 28))
	return holder
