-- Generated from assets/gameplay/tracks/kitchen/kitchen_track_metadata.json.
-- Do not hand-edit track values here; export metadata from the Godot authoring scene.
local M = {}

local kitchen = {
	audio_ids = {
		clatter = "res://assets/source/audio/canva/tracks/kitchen/kitchen_clatter_canva_01.mp3",
		music = "res://assets/source/audio/suno/tracks/kitchen/kitchen_loop_suno_01.mp3",
		sink_splash = "res://assets/source/audio/canva/tracks/kitchen/kitchen_sink_splash_canva_01.mp3",
		utensil_clink = "res://assets/source/audio/canva/tracks/kitchen/kitchen_utensil_clink_canva_01.wav",
	},
	audio_zones = {
		{
			audio_id = "clatter",
			audio_path = "",
			id = "FridgeHumZone",
			position = {
				126.0,
				8.0,
				34.0,
			},
			radius = 26.0,
			volume_db = -16.0,
			zone_kind = "ambient",
		},
		{
			audio_id = "clatter",
			audio_path = "",
			id = "KitchenClatterZone",
			position = {
				0.0,
				5.0,
				0.0,
			},
			radius = 120.0,
			volume_db = -12.0,
			zone_kind = "ambient",
		},
		{
			audio_id = "sink_splash",
			audio_path = "",
			id = "SinkSplashZone",
			position = {
				-8.0,
				4.45,
				74.0,
			},
			radius = 18.0,
			volume_db = -5.0,
			zone_kind = "oneshot",
		},
	},
	checkpoint_radius = 7.8,
	checkpoints = {
		{
			index = 0,
			is_lap_gate = true,
			position = {
				-86.0,
				3.34999990463257,
				-76.0,
			},
			route_index = 0,
		},
		{
			index = 1,
			is_lap_gate = false,
			position = {
				56.0,
				4.07999992370605,
				-8.0,
			},
			route_index = 8,
		},
		{
			index = 2,
			is_lap_gate = false,
			position = {
				86.0,
				5.69999980926514,
				-68.0,
			},
			route_index = 16,
		},
		{
			index = 3,
			is_lap_gate = false,
			position = {
				44.0,
				3.79999995231628,
				78.0,
			},
			route_index = 25,
		},
		{
			index = 4,
			is_lap_gate = false,
			position = {
				-96.0,
				4.09999990463257,
				56.0,
			},
			route_index = 30,
		},
		{
			index = 5,
			is_lap_gate = false,
			position = {
				-88.0,
				3.5,
				-22.0,
			},
			route_index = 34,
		},
	},
	closed_loop = true,
	definition_path = "res://assets/gameplay/tracks/kitchen/kitchen_track_definition.tres",
	display_name = "Kitchen / Sir Clink",
	hazard_sockets = {
		{
			position = {
				74.0,
				4.30000019073486,
				-48.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				-29.0,
				5.96000003814697,
				-34.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				126.0,
				15.25,
				24.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				-8.0,
				4.44999980926514,
				72.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				-96.0,
				4.30000019073486,
				18.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				-104.0,
				4.09999990463257,
				-50.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				58.0,
				4.25,
				-68.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				106.0,
				9.80000019073486,
				74.0,
			},
			yaw_degrees = 0.0,
		},
	},
	id = "kitchen",
	item_sockets = {
		{
			position = {
				-36.0,
				4.05000019073486,
				-78.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				24.0,
				4.17999982833862,
				-78.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				70.0,
				4.25,
				-66.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				14.0,
				5.42000007629395,
				0.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				90.0,
				6.21999979019165,
				-66.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				124.0,
				10.3999996185303,
				-18.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				126.0,
				15.4499998092651,
				36.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				20.0,
				4.25,
				76.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				-48.0,
				4.80000019073486,
				74.0,
			},
			yaw_degrees = 0.0,
		},
		{
			position = {
				-98.0,
				4.11999988555908,
				-40.0,
			},
			yaw_degrees = 0.0,
		},
	},
	lap_gate_checkpoint_index = 0,
	laps = 3,
	metadata_path = "res://assets/gameplay/tracks/kitchen/kitchen_track_metadata.json",
	out_of_bounds_y = 1.5,
	reset_mode = "instant_pop",
	floor_visual_y = -8.0,
	road_width = 12.0,
	route_length = 1016.6020154953,
	route_points = {
		{
			-86.0,
			3.34999990463257,
			-76.0,
		},
		{
			-56.0,
			3.34999990463257,
			-78.0,
		},
		{
			-24.0,
			3.34999990463257,
			-78.0,
		},
		{
			12.0,
			3.42000007629395,
			-78.0,
		},
		{
			46.0,
			3.48000001907349,
			-76.0,
		},
		{
			66.0,
			3.53999996185303,
			-68.0,
		},
		{
			76.0,
			3.61999988555908,
			-50.0,
		},
		{
			72.0,
			3.8199999332428,
			-26.0,
		},
		{
			56.0,
			4.07999992370605,
			-8.0,
		},
		{
			30.0,
			4.44999980926514,
			2.0,
		},
		{
			-2.0,
			4.84999990463257,
			0.0,
		},
		{
			-24.0,
			5.19999980926514,
			-16.0,
		},
		{
			-32.0,
			5.44999980926514,
			-42.0,
		},
		{
			-18.0,
			5.65000009536743,
			-62.0,
		},
		{
			16.0,
			5.78000020980835,
			-70.0,
		},
		{
			52.0,
			5.84999990463257,
			-70.0,
		},
		{
			86.0,
			5.69999980926514,
			-68.0,
		},
		{
			108.0,
			6.80000019073486,
			-52.0,
		},
		{
			122.0,
			9.19999980926514,
			-28.0,
		},
		{
			128.0,
			12.3000001907349,
			-2.0,
		},
		{
			128.0,
			14.8000001907349,
			24.0,
		},
		{
			128.0,
			14.8000001907349,
			46.0,
		},
		{
			122.0,
			13.1999998092651,
			66.0,
		},
		{
			104.0,
			9.19999980926514,
			76.0,
		},
		{
			72.0,
			5.59999990463257,
			78.0,
		},
		{
			44.0,
			3.79999995231628,
			78.0,
		},
		{
			12.0,
			3.70000004768372,
			76.0,
		},
		{
			-12.0,
			3.90000009536743,
			70.0,
		},
		{
			-40.0,
			4.09999990463257,
			76.0,
		},
		{
			-72.0,
			4.19999980926514,
			70.0,
		},
		{
			-96.0,
			4.09999990463257,
			56.0,
		},
		{
			-104.0,
			3.95000004768372,
			30.0,
		},
		{
			-90.0,
			3.77999997138977,
			14.0,
		},
		{
			-104.0,
			3.61999988555908,
			-4.0,
		},
		{
			-88.0,
			3.5,
			-22.0,
		},
		{
			-104.0,
			3.42000007629395,
			-44.0,
		},
		{
			-110.0,
			3.35999989509583,
			-62.0,
		},
		{
			-102.0,
			3.34999990463257,
			-74.0,
		},
	},
	runtime_scene_path = "res://assets/gameplay/tracks/kitchen/kitchen_track.tscn",
	shortcut_gates = {
		{
			entry = {
				70.0,
				3.58,
				-66.0,
			},
			exit = {
				14.0,
				5.38,
				0.0,
			},
			id = "table_jump",
			kind = "jump",
			surface_enabled = false,
			width = 8.0,
		},
	},
	spawn_points = {
		{
			position = {
				-82.0,
				4.15000009536743,
				-79.0,
			},
			yaw_degrees = 90.0,
		},
		{
			position = {
				-82.0,
				4.15000009536743,
				-74.0,
			},
			yaw_degrees = 90.0,
		},
		{
			position = {
				-76.0,
				4.15000009536743,
				-79.0,
			},
			yaw_degrees = 90.0,
		},
		{
			position = {
				-76.0,
				4.15000009536743,
				-74.0,
			},
			yaw_degrees = 90.0,
		},
		{
			position = {
				-70.0,
				4.15000009536743,
				-79.0,
			},
			yaw_degrees = 90.0,
		},
		{
			position = {
				-70.0,
				4.15000009536743,
				-74.0,
			},
			yaw_degrees = 90.0,
		},
		{
			position = {
				-64.0,
				4.15000009536743,
				-79.0,
			},
			yaw_degrees = 90.0,
		},
		{
			position = {
				-64.0,
				4.15000009536743,
				-74.0,
			},
			yaw_degrees = 90.0,
		},
	},
	stage_props = {
		{
			asset_path = "",
			audio_material_id = "stone",
			box_color = {
				0.86,
				0.78,
				0.65,
				1.0,
			},
			box_size = {
				228.0,
				0.36,
				16.0,
			},
			collision_mode = "visual",
			gameplay_tag = "countertop",
			id = "BackCountertop",
			kind = "box",
			position = {
				0.0,
				2.95,
				84.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "",
			audio_material_id = "metal",
			box_color = {
				0.68,
				0.72,
				0.76,
				1.0,
			},
			box_size = {
				10.0,
				14.0,
				38.0,
			},
			collision_mode = "visual",
			gameplay_tag = "kitchen_landmark",
			id = "FridgeLandmark",
			kind = "box",
			position = {
				126.0,
				7.0,
				34.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "",
			audio_material_id = "stone",
			box_color = {
				0.86,
				0.78,
				0.65,
				1.0,
			},
			box_size = {
				234.0,
				0.36,
				16.0,
			},
			collision_mode = "visual",
			gameplay_tag = "countertop",
			id = "FrontCountertop",
			kind = "box",
			position = {
				0.0,
				2.95,
				-84.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "",
			audio_material_id = "drywall",
			box_color = {
				0.62,
				0.57,
				0.49,
				1.0,
			},
			box_size = {
				252.0,
				32.0,
				2.0,
			},
			collision_mode = "visual",
			gameplay_tag = "room_envelope",
			id = "KitchenBackWall",
			kind = "box",
			position = {
				0.0,
				16.0,
				98.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "",
			audio_material_id = "drywall",
			box_color = {
				0.8,
				0.76,
				0.68,
				1.0,
			},
			box_size = {
				292.0,
				1.0,
				200.0,
			},
			collision_mode = "visual",
			gameplay_tag = "room_envelope",
			id = "KitchenCeiling",
			kind = "box",
			position = {
				9.0,
				25.8,
				0.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "",
			audio_material_id = "stone",
			box_color = {
				0.88,
				0.8,
				0.68,
				1.0,
			},
			box_size = {
				140.0,
				0.36,
				92.0,
			},
			collision_mode = "visual",
			gameplay_tag = "countertop",
			id = "KitchenIslandCountertop",
			kind = "box",
			position = {
				22.0,
				2.95,
				-36.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "",
			audio_material_id = "drywall",
			box_color = {
				0.58,
				0.54,
				0.48,
				1.0,
			},
			box_size = {
				2.0,
				32.0,
				196.0,
			},
			collision_mode = "visual",
			gameplay_tag = "room_envelope",
			id = "KitchenLeftWall",
			kind = "box",
			position = {
				-128.0,
				16.0,
				0.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "",
			audio_material_id = "drywall",
			box_color = {
				0.58,
				0.54,
				0.48,
				1.0,
			},
			box_size = {
				2.0,
				32.0,
				196.0,
			},
			collision_mode = "visual",
			gameplay_tag = "room_envelope",
			id = "KitchenRightWall",
			kind = "box",
			position = {
				146.0,
				16.0,
				0.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "res://assets/source/kenney/furniture_kit/kitchenSink.glb",
			audio_material_id = "ceramic",
			box_color = {
				0.75,
				0.72,
				0.64,
				1.0,
			},
			box_size = {
				1.0,
				1.0,
				1.0,
			},
			collision_mode = "visual",
			gameplay_tag = "kitchen_landmark",
			id = "KitchenSink",
			kind = "scene",
			position = {
				-8.0,
				2.15,
				88.0,
			},
			scale = {
				10.5,
				10.5,
				10.5,
			},
			yaw_degrees = 180.0,
		},
		{
			asset_path = "",
			audio_material_id = "stone",
			box_color = {
				0.82,
				0.76,
				0.66,
				1.0,
			},
			box_size = {
				16.0,
				0.36,
				160.0,
			},
			collision_mode = "visual",
			gameplay_tag = "countertop",
			id = "LeftCountertop",
			kind = "box",
			position = {
				-116.0,
				2.95,
				-2.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
		{
			asset_path = "",
			audio_material_id = "stone",
			box_color = {
				0.82,
				0.76,
				0.66,
				1.0,
			},
			box_size = {
				16.0,
				0.36,
				160.0,
			},
			collision_mode = "visual",
			gameplay_tag = "countertop",
			id = "RightCountertop",
			kind = "box",
			position = {
				122.0,
				2.95,
				-2.0,
			},
			scale = {
				1.0,
				1.0,
				1.0,
			},
			yaw_degrees = 0.0,
		},
	},
	surface_segments = {
		{
			end_route_index = 16,
			id = "CountertopMain",
			position = {
				-26.0,
				3.7,
				-78.0,
			},
			start_route_index = 0,
			surface_audio_id = "countertop",
			surface_material_id = "stone",
		},
		{
			end_route_index = 23,
			id = "FridgeTop",
			position = {
				128.0,
				14.9,
				34.0,
			},
			start_route_index = 18,
			surface_audio_id = "fridge_top",
			surface_material_id = "metal",
		},
		{
			end_route_index = 29,
			id = "WetSink",
			position = {
				-8.0,
				4.2,
				74.0,
			},
			start_route_index = 25,
			surface_audio_id = "wet_sink",
			surface_material_id = "wet_stone",
		},
	},
	track_id = "kitchen",
	version = "kitchen_v2_2026_04_29",
	wall_height = 1.6,
	wall_thickness = 0.45,
}

M.tracks = {
	kitchen = kitchen,
	oval = kitchen,
	serpentine = kitchen,
}

function M.get(id)
	return M.tracks[id or "kitchen"] or kitchen
end

return M

