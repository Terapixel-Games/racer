...
func test_kitchen_metadata_matches_server_shape() -> void:
	...
	assert_equal((metadata.get("route_points", []) as Array).size(), 10, "Metadata should export route points")
	...
	assert_true(float(metadata.get("route_length", 0.0)) > 420.0, "Metadata should include countertop route length")
