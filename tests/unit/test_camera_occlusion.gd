...
func test_motion_blur_intensity_is_zero_below_threshold() -> void:
	var intensity := RaceController.motion_blur_intensity_for_speed(12.0, 24.0, 62.0, 0.36)
...
func test_motion_blur_intensity_caps_at_high_speed() -> void:
	var intensity := RaceController.motion_blur_intensity_for_speed(90.0, 24.0, 62.0, 0.36)
	assert_equal(intensity, 0.36, "High speed should cap at configured max blur")

func test_motion_blur_intensity_interpolates_mid_speed() -> void:
	var intensity := RaceController.motion_blur_intensity_for_speed(43.0, 24.0, 62.0, 0.36)
	assert_true(intensity > 0.0, "Mid speed should enable some motion blur")
	assert_true(intensity < 0.36, "Mid speed should not hit max blur")
	assert_true(absf(intensity - 0.18) <= 0.001, "Mid speed should map near half intensity")
