...
func test_package_size_audit_tracks_racer_optimization_savings() -> void:
	...
	assert_true(optimized_bytes > 150 * 1024 * 1024, "Optimized racer GLBs should be staged for runtime")
	...
	assert_true(int(audit.get("arkit_face_capture_glb_bytes", 0)) > 0, "ARKit face-capture racer variants should be reported separately from optimized LOD0 savings")
...
func test_package_size_audit_reports_web_export_resource_categories() -> void:
	...
	assert_true(int(category_bytes.get("racer_lod", 0)) >= 0, "Staged LOD racer export bytes should be reported even when sprite-backed LODs replace GLBs")
	assert_true(int(category_bytes.get("racer_lod_sprites", 0)) > 0, "Sprite-backed racer LOD export bytes should be measurable")
