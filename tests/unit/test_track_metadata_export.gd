...
func test_kitchen_metadata_matches_server_shape() -> void:
	...
	assert_equal((metadata.get("route_points", []) as Array).size(), 18, "Metadata should export route points")
	...
	assert_equal((metadata.get("item_sockets", []) as Array).size(), 8, "Metadata should export 8 item sockets")
	assert_equal((metadata.get("hazard_sockets", []) as Array).size(), 6, "Metadata should export 6 hazard sockets")
	...
	assert_true(float(metadata.get("route_length", 0.0)) > 500.0, "Metadata should include expanded countertop route length")
