...
func test_kitchen_metadata_matches_server_shape() -> void:
	...
	assert_equal((metadata.get("route_points", []) as Array).size(), 21, "Metadata should export route points")
	...
	assert_true(float(metadata.get("route_length", 0.0)) > 550.0, "Metadata should include expanded countertop route length")
