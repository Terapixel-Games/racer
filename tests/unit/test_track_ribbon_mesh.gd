...
func test_layered_collision_mesh_offsets_backup_layers_below_visible_surface() -> void:
	var road := RoadMeshScript.new()
	...
	assert_true(absf(collision_vertices[0].y - source_vertices[0].y) <= 0.001, "First collision layer should align to the visible road mesh surface")
	assert_true(absf(collision_vertices[source_vertices.size()].y - (source_vertices[0].y - 0.16)) <= 0.001, "Second collision layer should sit just below the visible road mesh")
	assert_true(absf(collision_vertices[source_vertices.size() * 2].y - (source_vertices[0].y - 0.32)) <= 0.001, "Final collision layer should stay shallow enough that racers cannot settle deep inside the track")
	road.queue_free()
